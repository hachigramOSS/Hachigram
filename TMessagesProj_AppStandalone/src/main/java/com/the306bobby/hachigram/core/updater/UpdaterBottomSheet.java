...
public class UpdaterBottomSheet extends BottomSheet {
    ...
    public UpdaterBottomSheet(Context context, Theme.ResourcesProvider resourcesProvider, boolean available, UpdaterUtils.Update update) {
        ...
        if (available) {
            ...
            downloadButton.setOnClickListener(v -> {
                if (!downloadButtonClicked) {
                    ...
                    UpdaterUtils.downloadApk(getContext(), update.downloadURL, "Hachigram " + update.version, downloadButton);
                    dismiss();
                }
            });
            ...
            if (update.isForce()) {
                setCancelable(false);
                isForce = true;
                HachigramCoreConfig.INSTANCE.setAutoOTA(true);
            }
            HachigramCoreConfig.INSTANCE.setForceFound(update.isForce());
        } else {
            ...
            installBetas.setTextAndCheckAndIcon(getString(R.string.UP_InstallBetas), HachigramCoreConfig.INSTANCE.getInstallBetas(), R.drawable.test_tube_solar, false);
            installBetas.setOnClickListener(v -> {
                HachigramCoreConfig.INSTANCE.setInstallBetas(!HachigramCoreConfig.INSTANCE.getInstallBetas());
            ...
            checkOnLaunch.setTextAndCheckAndIcon(getString(R.string.UP_Auto_OTA), HachigramCoreConfig.INSTANCE.getAutoOTA(), R.drawable.msg_retry, false);
            checkOnLaunch.setOnClickListener(v -> {
                HachigramCoreConfig.INSTANCE.setAutoOTA(!HachigramCoreConfig.INSTANCE.getAutoOTA());
                checkOnLaunch.setChecked(!checkOnLaunch.isChecked());
            });
            if (!HachigramCoreConfig.INSTANCE.getForceFound()) linearLayout.addView(checkOnLaunch);
        ...
        if (available && !isForce) {
            ...
            scheduleButton.setOnClickListener(v -> {
                HachigramCoreConfig.INSTANCE.setUpdateScheduleTimestamp(System.currentTimeMillis());
                dismiss();
            });
    ...
    private void openGithubReleases() {
        String githubLink;
        if (HachigramCoreConfig.isStandaloneBetaBuild()) {
