...
import com.the306bobby.hachigram.helpers.network.NetworkHelper;
...
public class UpdaterUtils {
    ...
    private static final String RELEASES_API = "https://api.github.com/repos/hachigramOSS/Hachigram/releases";
    private static final String RELEASE_URI = RELEASES_API + "/latest";
    // Includes pre-releases, which /latest omits; newest first.
    private static final String BETA_URI = RELEASES_API + "?per_page=20";
    ...
    public static boolean updateFileExists() {
        version = HachigramCoreConfig.INSTANCE.getUpdateVersionName();
        ...
        HachigramCoreConfig.INSTANCE.setUpdateAvailable(isNew);
    ...
    public static void checkUpdates(BaseFragment fragment, boolean manual, OnUpdateNotFound onUpdateNotFound, OnUpdateFound onUpdateFound, Browser.Progress progress) {
        if (HachigramCoreConfig.isStandalonePremiumBuild()) return;
        ...
        if (!manual) {
            ...
            if (now - HachigramCoreConfig.INSTANCE.getUpdateScheduleTimestamp() < updateCheckInterval) return;
            if (now - HachigramCoreConfig.INSTANCE.getLastUpdateCheckTime() < updateCheckInterval) return;
        }

        checkingForUpdates = true;
        otaQueue.postRunnable(() -> {
            HachigramCoreConfig.INSTANCE.getLastUpdateCheckTime();
            HachigramCoreConfig.INSTANCE.setLastUpdateCheckTime(System.currentTimeMillis());
            try {
                boolean wantsPreReleases = HachigramCoreConfig.INSTANCE.getInstallBetas()
                ...
                for (int i = 0; i < arr.length(); i++) {
                    ...
                    downloadURL = link;
                    if (HachigramCoreConfig.isDevBuild()) FileLog.d("DownloadLink: " + downloadURL);
                ...
                if (update.isNew() && fragment != null && fragment.getContext() != null) {
                    ...
                    HachigramCoreConfig.INSTANCE.setUpdateIsDownloading(false);
                    HachigramCoreConfig.INSTANCE.setUpdateAvailable(true);
                    if (version != null && !version.equals("0")) HachigramCoreConfig.INSTANCE.setUpdateVersionName(version);
                    HachigramCoreConfig.INSTANCE.setUpdateSize(size);
                } else {
    ...
    public static void downloadApk(Context context, String link, String title, ButtonWithCounterView progressTextView) {
        if (context != null && !updateDownloaded) {
            ...
            HachigramCoreConfig.INSTANCE.setUpdateIsDownloading(true);
                    ...
                    if (cursor != null && cursor.moveToFirst()) {
                        ...
                        if (indexBytesDownloaded >= 0 && indexBytesTotal >= 0) {
                            ...
                            if (bytesTotal > 0) {
                                ...
                                HachigramCoreConfig.INSTANCE.setUpdateDownloadingProgress(progress);
                                if (HachigramCoreConfig.isDevBuild()) FileLog.e("Загрузка: " + progress + "%");
                            }
                        }
                    }
                } catch (Exception e) {
    ...
    public static void cancelDownload(Context context, long downloadId) {
        ...
        HachigramCoreConfig.INSTANCE.setUpdateIsDownloading(false);
        HachigramCoreConfig.INSTANCE.setUpdateDownloadingProgress(0f);
        NotificationCenter.getGlobalInstance().postNotificationName(NotificationCenter.appUpdateAvailable);
    }
    ...
    public static class Update {
        ...
        public boolean isNew() {
            ...
            HachigramCoreConfig.INSTANCE.setUpdateAvailable(offer);
            return offer;
        }
    ...
    /*public static String getLastCheckUpdateTime() {
        return getString(R.string.UP_LastCheck) + ": " + LocaleController.formatDateTime(HachigramCoreConfig.INSTANCE.getLastUpdateCheckTime() / 1000, true);
    }*/
