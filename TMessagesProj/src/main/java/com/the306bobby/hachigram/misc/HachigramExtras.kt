...
object HachigramExtras : CoroutineScope by MainScope() {
    ...
    fun checkChannelFollow(activity: Activity, currentAccount: Int) = AndroidUtilities.runOnUIThread {
        ...
        if (updateChannel is TLRPC.Chat) {
            launch(Dispatchers.IO) {
                if (updateChannel.id != Constants.Hachigram_Channel) {
    ...
    private fun checkChannelFollow(activity: Activity, currentAccount: Int, channel: TLRPC.Chat) {
        ...
        launch(Dispatchers.IO) {
            if (channel.id != Constants.Hachigram_Channel) {
...
//            builder.setNegativeButton(getString(R.string.Cancel), null)
    ...
    fun requestReviewFlow(fragment: BaseFragment?) {
        ...
        requestTask.addOnCompleteListener { request ->
            ...
            if (!fragment.messagesController.mainSettings.getBoolean("is_hachigram_rated", false)) {
                if (request.isSuccessful) {
                    ...
                    flow.addOnCompleteListener {
                        ...
                        fragment.messagesController.mainSettings.edit {
                            putBoolean("is_hachigram_rated", true)
                        }
                    }
                } else {
    ...
    fun isEdgeToEdgeSupported(): Boolean {
        if (HachigramCoreConfig.edgeToEdgeMode == HachigramCoreConfig.EDGE_MODE_ENABLE) {
            return true
        } else {
            isEdgeToEdgeCached?.let { return it }

            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.R || HachigramCoreConfig.edgeToEdgeMode == HachigramCoreConfig.EDGE_MODE_DISABLE) {
