...
import com.the306bobby.hachigram.chats.gemini.network.ApiClient;
import com.the306bobby.hachigram.chats.gemini.network.ModelInfo;
...
public class GeminiPreferencesEntry extends UniversalFragment {
    ...
    protected void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        ...
        if (!TextUtils.isEmpty(HachigramMessagesConfig.INSTANCE.getGeminiApiKey())) {
            geminiApiKeyField.getEditText().setText(HachigramMessagesConfig.INSTANCE.getGeminiApiKey());
        }
        ...
        geminiModelNameField.getEditText().setText(HachigramMessagesConfig.INSTANCE.getGeminiModelName());
        ...
        geminiSystemPromptField.getEditText().setText(HachigramMessagesConfig.INSTANCE.getGeminiSystemPrompt());
        ...
        items.add(
                UItem.asIntSlideView(
                        1,
                        1,
                        HachigramMessagesConfig.INSTANCE.getGeminiTemperatureValue(),
                        10,
                        val -> String.format(Locale.US, "%.1f", val / 10f),
                        HachigramMessagesConfig.INSTANCE::setGeminiTemperatureValue
                )
        );
    ...
    protected void onClick(UItem item, View view, int position, float x, float y) {
        if (item.id == geminiModelsListButton) {
            ApiClient.fetchModels(
                    getContext(),
                    getResourceProvider(),
                    HachigramMessagesConfig.INSTANCE.getGeminiApiKey(),
                    models -> AndroidUtilities.runOnUIThread(() -> {
                        ...
                        PopupHelper.show(getString(R.string.CP_GeminiAI_Model), modelFullTitleArray, modelDescriptionArray, modelShortTitleArray.indexOf(HachigramMessagesConfig.INSTANCE.getGeminiModelName()), getContext(), i -> {
                            HachigramMessagesConfig.INSTANCE.setGeminiModelName(modelShortTitleArray.get(i));
    ...
    private void doOnDone() {
        ...
        HachigramMessagesConfig.INSTANCE.setGeminiApiKey(
        ...
        HachigramMessagesConfig.INSTANCE.setGeminiModelName(
                geminiModelNameField.getEditText().getText().toString()
        );

        HachigramMessagesConfig.INSTANCE.setGeminiSystemPrompt(
