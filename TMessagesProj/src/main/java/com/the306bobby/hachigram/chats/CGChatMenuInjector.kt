...
object CGChatMenuInjector {
    ...
    fun injectHachigramShortcuts(
        ...
        val requireBiometrics = HachigramPrivacyConfig.askBiometricsToOpenChat && !secretChat
        val isAnyButtonEnabled = requireBiometrics || HachigramChatsConfig.shortcut_JumpToBegin
                    || HachigramChatsConfig.shortcut_DeleteAll || HachigramChatsConfig.shortcut_SavedMessages
                    || HachigramChatsConfig.shortcut_Browser
        ...
        if (HachigramChatsConfig.shortcut_JumpToBegin) headerItem.lazilyAddSubItem(
        ...
        if (currentChat != null && !isDeleteAllHidden(currentChat) && (ChatObject.isMegagroup(currentChat) || !ChatObject.isChannel(currentChat))) {
            if (HachigramChatsConfig.shortcut_DeleteAll) headerItem.lazilyAddSubItem(
        ...
        if (currentChat != null && currentChat.id != abs(ChatsHelper2.getCustomChatID())
            ...
            if (HachigramChatsConfig.shortcut_SavedMessages) headerItem.lazilyAddSubItem(
        ...
        if (HachigramChatsConfig.shortcut_Browser) headerItem.lazilyAddSubItem(
    ...
    fun injectAdminShortcuts(headerItem: ActionBarMenuItem, currentChat: TLRPC.Chat) {
        val isAnyButtonEnabled = HachigramChatsConfig.admins_Reactions || HachigramChatsConfig.admins_Permissions || HachigramChatsConfig.admins_Administrators
                || HachigramChatsConfig.admins_Members || HachigramChatsConfig.admins_Statistics || HachigramChatsConfig.admins_RecentActions

        if (isAnyButtonEnabled) headerItem.lazilyAddColoredGap()

        if (HachigramChatsConfig.admins_Reactions && ChatObject.canChangeChatInfo(currentChat)) headerItem.lazilyAddSubItem(
                ChatActivityHelper.OPTION_FOR_ADMINS_REACTIONS,
                R.drawable.msg_reactions2,
                getString(R.string.Reactions)
        )

        if (HachigramChatsConfig.admins_Permissions && !(ChatObject.isChannel(currentChat) && !currentChat.megagroup) && !currentChat.gigagroup) headerItem.lazilyAddSubItem(
                ChatActivityHelper.OPTION_FOR_ADMINS_PERMISSIONS,
                R.drawable.msg_permissions,
                getString(R.string.ChannelPermissions)
        )

        if (HachigramChatsConfig.admins_Administrators) headerItem.lazilyAddSubItem(
                ChatActivityHelper.OPTION_FOR_ADMINS_ADMINISTRATORS,
                R.drawable.msg_admins,
                getString(R.string.ChannelAdministrators)
        )

        if (HachigramChatsConfig.admins_Members) headerItem.lazilyAddSubItem(
        ...
        if (HachigramChatsConfig.admins_Statistics && ChatObject.isBoostSupported(currentChat)) headerItem.lazilyAddSubItem(
                ChatActivityHelper.OPTION_FOR_ADMINS_STATISTICS,
                R.drawable.msg_stats,
                getString(R.string.StatisticsAndBoosts)
        )

        if (HachigramChatsConfig.admins_RecentActions) headerItem.lazilyAddSubItem(
    ...
    private fun isDeleteAllHidden(chat: TLRPC.Chat): Boolean {
        return Constants.Hachigram_Support == chat.id
    }
    ...
    fun injectArchived(io: ItemOptions, fragment: BaseFragment) {
        io.addIf(
            !HachigramPrivacyConfig.hideArchiveFromChatsList && (HachigramAppearanceConfig.tabsHideAllChats || !MainTabsManager.hasTab(MainTabsManager.TabType.SETTINGS) || !HachigramAppearanceConfig.showMainTabs),
    ...
    fun injectSaved(io: ItemOptions, fragment: BaseFragment) {
        io.addIf(
            !MainTabsManager.hasTab(MainTabsManager.TabType.SETTINGS) || !HachigramAppearanceConfig.showMainTabs,
            R.drawable.msg_saved,
            getString(R.string.SavedMessages)
        ) {
    ...
    fun injectCalls(io: ItemOptions, fragment: BaseFragment) {
        io.addIf(
            !HachigramAppearanceConfig.showMainTabs || !MainTabsManager.hasTab(MainTabsManager.TabType.CALLS),
    ...
    fun injectGifts(io: ItemOptions, currentAccount: Int, context: Context) {
        val available: Boolean = HachigramAppearanceConfig.marketPlaceDrawerButton
