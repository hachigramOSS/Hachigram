...
package com.the306bobby.hachigram.chats.translator;
...
public class Translator {
    ...
    public static void showTranslationTargetSelector(Context context, boolean isKeyboard, Runnable callback, Theme.ResourcesProvider resourcesProvider) {
        ...
        PopupHelper.show(names, getString(R.string.CG_TranslationLanguage), targetLanguages.indexOf(isKeyboard ? HachigramMessagesConfig.INSTANCE.getTranslationKeyboardTarget() : HachigramMessagesConfig.INSTANCE.getTranslationTarget()), context, i -> {
            if (isKeyboard) {
                HachigramMessagesConfig.INSTANCE.setTranslationKeyboardTarget(targetLanguages.get(i));
            } else {
                HachigramMessagesConfig.INSTANCE.setTranslationTarget(targetLanguages.get(i));
            }
            callback.run();
        }, resourcesProvider);
    }
