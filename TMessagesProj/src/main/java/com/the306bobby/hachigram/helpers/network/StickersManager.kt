...
package com.the306bobby.hachigram.helpers.network
...
object StickersManager {
    ...
    private val REFRESH_INTERVAL = if (HachigramCoreConfig.isDevBuild()) 60 * 60 * 1000L else 24 * 60 * 60 * 1000L // 24 hours
    ...
    fun copyStickerFromAssets() {
        try {
            val outFile = File(ApplicationLoader.applicationContext.getExternalFilesDir(null), "stickers/hachigram.webm")
            if (outFile.exists()) return
            outFile.parentFile?.mkdirs()

            ApplicationLoader.applicationContext.assets.open("hachigram.webm").use { input ->
