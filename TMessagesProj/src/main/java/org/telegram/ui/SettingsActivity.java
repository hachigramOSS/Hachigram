...
//        fragmentView.addView(navigationBar, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, 0, Gravity.BOTTOM | Gravity.FILL_HORIZONTAL));
    ...
    public void openDebugMenu() {
        ...
        items = new CharSequence[]{
                ...
                BuildVars.DEBUG_PRIVATE_VERSION ? SharedConfig.disableVoiceAudioEffects ? UiCase.apply("Enable voip audio effects") : UiCase.apply("Disable voip audio effects") : null,
                BuildVars.DEBUG_PRIVATE_VERSION ? UiCase.apply("Clean app update") : null,
                BuildVars.DEBUG_PRIVATE_VERSION ? UiCase.apply("Reset suggestions") : null,
                ...
                AndroidUtilities.isTabletInternal() && BuildVars.DEBUG_PRIVATE_VERSION ? SharedConfig.forceDisableTabletMode ? UiCase.apply("Enable tablet mode") : UiCase.apply("Disable tablet mode") : null,
                ...
                BuildVars.DEBUG_PRIVATE_VERSION ? UiCase.apply("Force remove premium suggestions") : null,
                BuildVars.DEBUG_PRIVATE_VERSION ? UiCase.apply("Share device info") : null,
                BuildVars.DEBUG_PRIVATE_VERSION ? UiCase.apply("Force performance class") : null,
                BuildVars.DEBUG_PRIVATE_VERSION && !InstantCameraView.allowBigSizeCameraDebug() ? !SharedConfig.bigCameraForRound ? UiCase.apply("Force big camera for round") : UiCase.apply("Disable big camera for round") : null,
                getString(DualCameraView.dualAvailableStatic(getContext()) ? "DebugMenuDualOff" : "DebugMenuDualOn"),
                BuildVars.DEBUG_VERSION ? SharedConfig.useSurfaceInStories ? UiCase.apply("back to TextureView in stories") : UiCase.apply("use SurfaceView in stories") : null,
                BuildVars.DEBUG_PRIVATE_VERSION ? SharedConfig.photoViewerBlur ? UiCase.apply("do not blur in photoviewer") : UiCase.apply("blur in photoviewer") : null,
                !SharedConfig.payByInvoice ? UiCase.apply("Enable Invoice Payment") : UiCase.apply("Disable Invoice Payment"),
                BuildVars.DEBUG_PRIVATE_VERSION ? UiCase.apply("Update Attach Bots") : null,
                !SharedConfig.isUsingCamera2(currentAccount) ? UiCase.apply("Use Camera 2 API") : UiCase.apply("Use old Camera 1 API"),
                BuildVars.DEBUG_VERSION ? UiCase.apply("Clear Mini Apps Permissions and Files") : null,
                BuildVars.DEBUG_PRIVATE_VERSION ? UiCase.apply("Clear all login tokens") : null,
                SharedConfig.canBlurChat() && Build.VERSION.SDK_INT >= 31 ? SharedConfig.useNewBlur ? UiCase.apply("back to cpu blur") : UiCase.apply("use new gpu blur") : null,
                SharedConfig.adaptableColorInBrowser ? UiCase.apply("Disabled adaptive browser colors") : UiCase.apply("Enable adaptive browser colors"),
                SharedConfig.debugVideoQualities ? UiCase.apply("Disable video qualities debug") : UiCase.apply("Enable video qualities debug"),
                ...
                UiCase.apply("Reload app config"),
                !SharedConfig.forceForumTabs ? UiCase.apply("Force Forum Tabs") : UiCase.apply("Do Not Force Forum Tabs"),
                UiCase.apply("Make Memory Dump"),
                BuildVars.DEBUG_PRIVATE_VERSION ? (SharedConfig.fastWallpaperDisabled ? UiCase.apply("enable wallpaper shader") : UiCase.apply("disable wallpaper shader")) : null,
                (SharedConfig.frameMetricsEnabled ? UiCase.apply("hide frame metrics") : UiCase.apply("show frame metrics")),
                BuildVars.DEBUG_PRIVATE_VERSION ? (SharedConfig.shadowsInSections ? UiCase.apply("disable shadows in settings") : UiCase.apply("enable shadows in settings")) : null,
                BuildVars.DEBUG_PRIVATE_VERSION ? (SharedConfig.debugViewMetrics ? UiCase.apply("disable debug view metrics") : UiCase.apply("enable debug view metrics")) : null,
        };
